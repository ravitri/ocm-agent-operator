...
make -C ${GIT_ROOT} ocm-agent-build
...
if [[ -n $FLEET_MODE ]]; then
    ...
    ${GIT_ROOT}/build/ocm-agent/_output/bin/ocm-agent serve --services "$SERVICE" --ocm-url "$OCM_BASE_URL" $FLEET_MODE --ocm-client-id "$OA_OCM_CLIENT_ID" --ocm-client-secret "$OA_OCM_CLIENT_SECRET"
else
    ...
    ${GIT_ROOT}/build/ocm-agent/_output/bin/ocm-agent serve --access-token "$ACCESS_TOKEN" --services "$SERVICE" --cluster-id "$EXT_CLUSTERID" --ocm-url "$OCM_BASE_URL"
fi
